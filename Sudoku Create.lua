...
function resolvSudoku1(tableau,line,column)
    tmpre = 0
    for line = line, 9 do
        if tmpre == 2 then
            column = 1
        ...
        for column = column, 9 do
            if tableau[line][column] == "-" then
                for case = 1, 9 do
                    ...
                    tmp = checkValid(tableau,line,column)
                    if  tmp == true then
                    	relosv = resolvSudoku1(tableau,line,column+1)
                        if relosv == true then
                            return false
                        else if relosv == tableau then
                        	return tableau
...
function sudokutotable1(tableau)
    for n = 1, 9 do
        for b = 1, 9 do
            if checkValid(tableau,n,b) then
            else
	...
	--printtableau(tableau)
    finishtableau = resolvSudoku1(tableau,1,1)
    return finishtableau
end

function sudokutotablerev(tableau)
    for n = 1, 9 do
        for b = 1, 9 do
            if checkValid(tableau,n,b) then
            else
	...
	--printtableau(tableau)
    finishtableau = resolvSudokurev(tableau,9,9)
    return finishtableau
end

function sudoku1(tableau)
	term.clear()
	term.setCursorPos(1,1)
	blue = sudokutotable1(tableau)
	if blue == false then
	    term.clear()
	    term.setCursorPos(1,1)
	    term.write("Error No Valid Sudoku.")
...
function sudokurev(arg)
	term.clear()
	term.setCursorPos(1,1)
	blue = sudokutotablerev(arg)
	if blue == false then
	    term.clear()
	    term.setCursorPos(1,1)
	    term.write("Error No Valid Sudoku.")
...
function compasudo(tableau1,tableaurev)
	g = 0
	for i = 1, 9 do
		for j = 1, 9 do
			if tonumber(tableau1[i][j]) == tonumber(tableaurev[i][j]) then
			else
				return false
			end
		end
...
function res(tableau)
	tableau1 = sudoku1(tableau)
	tableaurev = sudokurev(tableau)
	if compasudo(tableau1,tableaurev) == true then
		return true
	else
		return false
	end
...
function start()
	while true do
		term.clear()
		term.setCursorPos(1,1)
		term.write("Difficulter entre 1 et 6 : ")
		g = 1
		h = 1
		diff = tonumber(read())
		if diff == 1 then
